...
if (!empty($_GET['id'])) {
    ...
    $stmtKelas = $conn->prepare("SELECT tbsppsiswa.kodekelas, tbspp.TahunAjaran FROM tbsppsiswa JOIN tbkelas ON tbkelas.KodeKelas = tbsppsiswa.kodekelas JOIN tbspp ON tbkelas.KodeSPP = tbspp.KodeSPP WHERE tbsppsiswa.kode_spp_siswa = ?");
